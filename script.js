...
/* ---- Sticky Header ---- */
/* Perf: rAF-throttled + passive so this never blocks native scroll compositing
   and never runs more than once per rendered frame. */
...
/* ---- About Section — Profile Photo Switch ---- */
const ABOUT_PHOTOS = [
...
/* ---- Active nav ---- */
/* Perf: the old handler read s.offsetTop for every section inside the scroll
   listener, forcing a synchronous layout recalculation on every single native
   scroll event — this was the main cause of the Hero scroll jank/stutter.
   Fix: cache each section's offsetTop once (recomputed on resize, so
   responsive breakpoints stay correct), then the scroll handler only reads
   window.scrollY (which never forces layout) and is rAF-throttled + passive
   so it runs at most once per frame and never blocks native scrolling. Nav
   link styles are only written when the active section actually changes,
...
function drawWebThumb() {
  ...
  [['HTML','rgba(201,168,76,0.15)','rgba(201,168,76,0.4)','#C9A84C'],
   ['CSS','rgba(46,204,113,0.12)','rgba(46,204,113,0.4)','#779EC5'],
...
function enterPortfolio() {
  ...
  // Start the background audio (triggered by this click = user interaction).
  // There's no player UI to update — this only needs to request playback,
  // it never assumes success (see the 'error' listener on #bgMusic below).
...
/* ---- Music (background audio only — no visible player UI) ----
   The <audio id="bgMusic"> element itself is the only thing that remains;
   there is no Play/Pause button or label to keep in sync anymore. */
const music = document.getElementById('bgMusic');

if (music) {
  music.addEventListener('error', () => {
    // Fires on 404 / unsupported format / decode / network errors.
    // Never breaks the page — playback just silently doesn't start.
...
/* Reusable: stop background music on demand (used by the CV links below,
   since there is no longer a Play/Pause control for the visitor to use). */
...
/* ---- Hero stat: project count synced dynamically from Portfolio DOM ---- */
(function () {
...
/* ---- Job Status Indicator: blinking dot next to Hero "CV Universal" ---- */
